...
class InterestsFactsListView extends StatelessWidget {
  // 1
  final String username;
  final List<Interest> interests;
  final List<KeyValue> facts;

  const InterestsFactsListView(
      {Key? key,
  ...
  Widget interestFactList() {
    return ListView.builder(
      itemCount: interests.length + facts.length,
      shrinkWrap: true,
          ...
          if (index == 0) {
            ...
            return Column(
              ...
              children: [
                ...
                factTag(context, facts[index])
              ],
            );
          } else {
            return factTag(context, facts[index]);
          }
          return Container();
        } else {
          //Interests section
          int iindex = index - facts.length;
          if (index == facts.length && interests.length > 0) {
            //Section Header
            return Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
  ...
  Widget UsernameSection(String username) {
    return Column(
      children: [
        SizedBox(height: 20),
        Container(
